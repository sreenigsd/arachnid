...
import com.gsd.sreenidhi.arachnid.model.HttpRequestBody;
import com.gsd.sreenidhi.arachnid.proxy.Proxy;
import com.gsd.sreenidhi.arachnid.proxy.SimpleProxyProvider;
import com.gsd.sreenidhi.arachnid.selector.Html;
import com.gsd.sreenidhi.arachnid.utils.CharsetUtils;
...
public class HttpClientDownloaderTest {
    ...
    public void testDownloader() {
        ...
        Html html = httpClientDownloader.download("https://www.github.com/");
    ...
    public void testDownloaderInIllegalUrl() throws UnsupportedEncodingException {
        HttpClientDownloader httpClientDownloader = new HttpClientDownloader();
        httpClientDownloader.download("http://www.github.net/>");
    }
    ...
    public void test_selectRequestMethod() throws Exception {
        ...
        server.get(eq(query("q"), "arachnid")).response("get");
        server.post(eq(form("q"), "arachnid")).response("post");
        server.put(eq(form("q"), "arachnid")).response("put");
        server.delete(eq(query("q"), "arachnid")).response("delete");
        server.request(and(by(method("HEAD")),eq(query("q"), "arachnid"))).response(header("method","head"));
        server.request(and(by(method("TRACE")),eq(query("q"), "arachnid"))).response("trace");
        ...
        Runner.running(server, new Runnable() {
            ...
            public void run() throws Exception {
                ...
                request.setUrl("http://127.0.0.1:" + port + "/search?q=arachnid");
                request.setMethod(HttpConstant.Method.GET);
                Map<String,Object> params = new HashedMap();
                params.put("q","arachnid");
    ...
    public void test_set_request_cookie() throws Exception {
        HttpServer server = httpServer(13423);
        server.get(eq(cookie("cookie"), "cookie-arachnid")).response("ok");
        Runner.running(server, new Runnable() {
            @Override
            public void run() throws Exception {
                HttpClientDownloader httpClientDownloader = new HttpClientDownloader();
                ...
                request.addCookie("cookie","cookie-arachnid");
    ...
    public void test_disableCookieManagement() throws Exception {
        HttpServer server = httpServer(13423);
        server.get(not(eq(cookie("cookie"), "cookie-arachnid"))).response("ok");
        Runner.running(server, new Runnable() {
            @Override
            public void run() throws Exception {
                HttpClientDownloader httpClientDownloader = new HttpClientDownloader();
    ...
    public void test_set_request_header() throws Exception {
        HttpServer server = httpServer(13423);
        server.get(eq(header("header"), "header-arachnid")).response("ok");
        Runner.running(server, new Runnable() {
            @Override
            public void run() throws Exception {
                HttpClientDownloader httpClientDownloader = new HttpClientDownloader();
                Request request = new Request();
                request.setUrl("http://127.0.0.1:13423");
                request.addHeader("header","header-arachnid");
                Page page = httpClientDownloader.download(request, Site.me().toTask());
                assertThat(page.getRawText()).isEqualTo("ok");
            }
        });
    ...
    public void test_set_site_header() throws Exception {
        ...
        Runner.running(server, new Runnable() {
            ...
            public void run() throws Exception {
                ...
                Request request = new Request();
                request.setUrl("http://127.0.0.1:13423");
                Page page = httpClientDownloader.download(request, Site.me().addHeader("header","header-arachnid").toTask());
                assertThat(page.getRawText()).isEqualTo("ok");
            }
        });
    }
    ...
    public void test_set_site_cookie() throws Exception {
        ...
        Runner.running(server, new Runnable() {
            ...
            public void run() throws Exception {
                ...
                Request request = new Request();
                request.setUrl("http://127.0.0.1:13423");
                Site site = Site.me().addCookie("cookie", "cookie-arachnid").setDomain("127.0.0.1");
    ...
    public void test_download_auth_by_SimpleProxyProvider() throws Exception {
        ...
        Runner.running(server, new Runnable() {
            ...
            public void run() throws Exception {
                ...
                Request request = new Request();
                request.setUrl("http://www.github.com");
                Page page = httpClientDownloader.download(request, Site.me().toTask());
                assertThat(page.getRawText()).isEqualTo("ok");
            }
        });
